...
i = 0
test_ecb_data.each do |test_item|
  blowcypher = JdCrypt::Blowfish.new([test_item[0]].pack("H*"))
  cyphertext = blowcypher.encrypt([test_item[1]].pack("H*"))
  if cyphertext != [test_item[2]].pack("H*") || die_please == i
    p "#{i} key=#{test_item[0]}, ptext=#{test_item[1]}: #{test_item[2]} != #{cyphertext.unpack1("H*")}"
    $DEBUG = 1
...
test_ecb_data.each do |test_item|
  ...
  if plaintext != [test_item[1]].pack("H*") || die_please == i
    p "#{i} key=#{test_item[0]}, ctext=#{test_item[2]}: #{test_item[1]} != #{plaintext.unpack1("H*")}"
    $DEBUG = 1
